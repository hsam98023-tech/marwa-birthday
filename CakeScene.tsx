...
// رديناها const عادية باش نديرو ليها export default لتحت
const CakeScene: React.FC<CakeSceneProps> = ({ onComplete }) => {
  const [isBlown, setIsBlown] = useState(false);

  const particles = Array.from({ length: 60 }).map((_, i) => ({
    id: i,
    x: (Math.random() - 0.5) * 600,
    y: (Math.random() - 1.2) * 500,
...
// هاد السطر هو اللي كيحل المشكل مع App.tsx
